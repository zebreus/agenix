...
**Required.** Path to the directory containing `secrets.nix` and all secret files. Secret files are resolved as `${secretsPath}/${name}.age` and public files as `${secretsPath}/${name}.pub`; there is no per-secret file option.

[source,nix]
----
{
  # All secrets resolve to files in ./secrets/
...
NOTE: Secret files must be regular files. Symlinks are rejected at evaluation time, because importing a symlink into the Nix store would break it.

=== age.secrets

Attribute set of secrets to decrypt. Each key is a secret name from `secrets.nix`, each value is the secret's installation configuration.
...
Name of the secret in `secrets.nix` and of the decrypted file in the secrets directory. Default: the attribute name. Setting it lets one secret file be installed under several attribute names with different configurations.

[source,nix]
----
{
  age.secrets.my-alias = {
    name = "actual-secret";  # Decrypts ./secrets/actual-secret.age
  };
}
----
...
**Read-only.** Path to the public key file, resolved as `${secretsPath}/${name}.pub`.
...
|Derived from `secretsPath` (read-only)
|Settable; defaults to `${secretsPath}/${name}.age`
...
Same as NixOS module, but without `owner` and `group`, and with a settable `file` option (`age.secretsPath` is optional here).
