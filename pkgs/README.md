...
Options:
- `-i, --identity <KEY>` - Identity (private key) to use when decrypting
- `--partial` - Allow partial rekeying: continue even if some secrets cannot be decrypted
...
agenix rekey --partial                    # Continue rekeying even if some fail
```
...
- `--no-dependencies` - Do not generate dependencies of specified secrets (dependencies are generated by default)

When generating specific secrets, their dependencies are automatically generated by default.
If a dependency already has a `.pub` file from a previous generation, it will be used instead of regenerating.

Examples:
```bash
agenix generate                              # Generate all secrets
agenix generate secret1.age                  # Generate secret and its dependencies
agenix generate --dry-run                    # Preview what would be generated
agenix generate --force                      # Overwrite existing secrets
agenix generate --no-dependencies secret.age # Generate only specified secret (requires deps to exist)
```
