...
pub fn edit_file(
    ...
    if Path::new(file).exists() {
        ...
        {
            if force {
                info!("Warning: Could not decrypt {file}, starting with empty content: {e:#}");
            } else {
    ...
    if !cleartext_file.exists() {
        info!("Warning: {file} wasn't created");
        return Ok(());
    }
    ...
    {
        info!("Warning: {file} wasn't changed, skipping re-encryption");
        return Ok(());
    }
