use super::*;
use crate::nix::eval::{eval_nix_expression, value_to_string};
use crate::nix::keypair::{generate_age_x25519_keypair, generate_ed25519_keypair};
...
fn test_unicode_and_special_characters() -> Result<()> {
    ...
    let space_keys = get_public_keys(temp_file.path().to_str().unwrap(), "secret with spaces.age")?;
    ...
    let cyrillic_keys = get_public_keys(temp_file.path().to_str().unwrap(), "путь/к/секрету.age")?;
...
fn test_generate_age_keypair_with_public() -> Result<()> {
    ...
    let result = generate_secret_with_public(temp_file.path().to_str().unwrap(), "age-key.age")?;
...
fn test_auto_generator_password_ending() -> Result<()> {
    ...
    let result =
        generate_secret_with_public(temp_file.path().to_str().unwrap(), "database-password.age")?;
...
fn test_auto_generator_passphrase_ending() -> Result<()> {
    ...
    let result =
        generate_secret_with_public(temp_file.path().to_str().unwrap(), "backup-passphrase.age")?;
