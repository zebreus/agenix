...
*generate*, *g* [_SECRET_]...::
  Generate secrets using generator functions defined in `secrets.nix`. If no secrets are specified, generates all secrets that have generators.
  +
  Command options:::
    *-f*, *--force*::::
...
*builtins.wireguardKey {}*::
  WireGuard keypair. Returns `{ secret, public }` with base64-encoded Curve25519 keys.
...
|`*_wg.age`, `*_wireguard.age`
|`builtins.wireguardKey {}` (WireGuard keypair)
