...
decrypted_ssh=$(agenix -d server-ed25519.age --rules "$TMPDIR/auto-generate-test/auto-generate-secrets.nix" -i "$TEST_USER_KEY")
...
decrypted_x25519=$(agenix -d identity-x25519.age --rules "$TMPDIR/auto-generate-test/auto-generate-secrets.nix" -i "$TEST_USER_KEY")
...
decrypted_password=$(agenix -d database-password.age --rules "$TMPDIR/auto-generate-test/auto-generate-secrets.nix" -i "$TEST_USER_KEY")
