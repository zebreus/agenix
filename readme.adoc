...
----
{
  # Where the secrets live, and which one to decrypt
...
* Generate secrets using built-in generators (SSH keys, passwords, UUIDs, age keys, WireGuard keys)
...
in {
  "db-password".publicKeys = [ user host ];
}
----
...
[source,shell]
----
cd secrets && agenix edit db-password
----
